...
/** 页码指示器 */
@property (nonatomic,strong) ZDHPageControl *pageControl;
...
-(void)createPageControl{
    ...
    _pageControl = [[ZDHPageControl alloc]initWithFrame:CGRectMake((self.frame.size.width - 200)/2, ZXMainScrollViewHeight + 10, 200, 5)];
...
-(void)updateViewFrameSetting{
    ...
    //图片视图；左边
    self.leftIV.frame = CGRectMake(self.itemMargnPadding/2, self.imageHeightPoor*2, self.imgWidth, ZXMainScrollViewHeight-self.imageHeightPoor*2);
    //图片视图；中间
    self.centerIV.frame = CGRectMake(ZXMainScrollViewWidth + self.itemMargnPadding/2, 0.0, self.imgWidth, ZXMainScrollViewHeight);
    //图片视图；右边
    self.rightIV.frame = CGRectMake(ZXMainScrollViewWidth * 2.0 + self.itemMargnPadding/2, self.imageHeightPoor*2, self.imgWidth, ZXMainScrollViewHeight-self.imageHeightPoor*2);
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if (self.itemMargnPadding > 0) {
        ...
        if (currentX > 0) { //左滑
            ...
            self.centerIV.y = currentX/ZXMainScrollViewWidth*self.imageHeightPoor*2;
            self.rightIV.height = ZXMainScrollViewHeight-2*self.imageHeightPoor+variableH;
            self.rightIV.y = self.imageHeightPoor*2-currentX/ZXMainScrollViewWidth*self.imageHeightPoor*2;
            ...
            self.leftIV.y = self.imageHeightPoor*2+currentX/ZXMainScrollViewWidth*self.imageHeightPoor*2;
        } else {
            self.leftIV.alpha = self.initAlpha;
            self.centerIV.alpha = 1;
            self.rightIV.alpha = self.initAlpha;
            ...
            self.leftIV.y = self.imageHeightPoor*2;
            self.centerIV.y = 0;
            self.rightIV.y = self.imageHeightPoor*2;
        }
    }
}
@end
